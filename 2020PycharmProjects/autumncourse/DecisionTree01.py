...
def Decision_Tree(data, root_node, old_features_list, entD):
    ...
    for feature in features_list:  # 对于不同属性
        ...
        for per_plist in plist:
            per_ent = Info_Entropy(per_plist)
        ...
        ent_whole_step1.append(ent_whole)  # 该属性几个子类的信息熵
        ent_whole_name.append(fe)          # 该属性几个子类的信息熵对应的名字，如‘清晰’
...
def save_re(features_list, gain_step):
    ...
    re = np.hstack(
        (features_listcp.reshape((features_listcp.shape[0], 1)), gain_stepcp.reshape((features_listcp.shape[0], 1))))
    return pd.DataFrame(re, columns=['属性', '信息增益'])


def main():
    data = pd.read_excel(r'F:\ProgrammingWorkspace\PycharmProjections\2020PycharmProjects\autumncourse\watermelon20.xlsx')
